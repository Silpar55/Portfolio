        ...
        {imgUrl && (
          <Image
            ...
            className={`object-cover h-auto sm:w-2/3 z-0 opacity-70 mt-5 md:w-2/5 ${
              slideNumber == 3 && " bg-slate-600 rounded transparent"
            } ${
              slideNumber == 4 && "h-80 xl:h-96 w-auto sm:w-auto md:w-auto"
            } ${slideNumber == 5 && "h-32 w-auto sm:w-auto md:w-auto"}`}
          />
        )}
        {imgUrl2 && (
          <Image
          ...
          ${slideNumber == 5 && "w-1/4 md:w-1/4"}`}
          />
        )}
      </div>
      <p className="text-sm absolute bottom-2 text-white">
        Created by: Alejandro Silva Juárez © 2023 - All rights reserved
      </p>
    </>
  );
};
